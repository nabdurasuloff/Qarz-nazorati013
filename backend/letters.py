...
def _shablon_papkasi():
    ...
    DOIMIY papka — exe bilan bir joyda, hech qachon o'chmaydi/vaqtinchalik
    bo'lmaydi (PyInstaller --onefile'ning vaqtinchalik _MEIPASS papkasidan
    farqli o'laroq). Agar bu yerda moslashtirilgan shablon bo'lsa, u standart
    (dastur ichidagi) shablondan USTUN turadi."""
    if getattr(sys, 'frozen', False):
        papka = os.path.join(os.path.dirname(os.path.abspath(sys.executable)), 'moslashtirilgan_shablonlar')
    else:
...
def generate_reestr_ssp(output_path, mijozlar_royxati, xat_raqami, xat_sanasi, settings):
    ...
    """
    if not os.path.exists(REESTR_SSP_TEMPLATE_PATH):
        raise FileNotFoundError(
    ...
    doc = Document(REESTR_SSP_TEMPLATE_PATH)
